...
- The interface reaches Rust through four named read commands and one recording-status event, and through nothing else. It cannot run a shell command, act on any path, or call anything its capability files do not grant; those files are scoped to the one window by name and grant no filesystem, shell, network, process, or path permission. It is handed two paths — the configuration file and the storage root — because a settings view has to render them, and diagnostic messages routinely quote them; nothing in the grant set can open, write, or list any of them. It also cannot navigate away from the application's own origin, so what it is handed cannot be carried off in a URL. `pnpm --dir scrybe-desktop run check:capabilities` rejects a capability that reaches any of them, and `pnpm --dir scrybe-desktop run check:bindings` fails when the TypeScript the interface imports no longer matches the Rust it was generated from.
...
- A managed model catalog. `scrybe-application/models.toml` records the one model the application offers to install — its stable identity, an artifact URL pinned to an immutable upstream revision, the licence, the exact byte count, the SHA-256 digest, the runtime that loads it, and the filename it is promoted to — and the catalog is validated rather than trusted: an entry served over plain HTTP, carrying a URL its own revision does not appear in, holding something that is not a lowercase SHA-256, or naming a destination that is a path rather than a filename fails a test instead of a download.
- Model acquisition is confirmed, cancellable, verified, and atomic. Nothing is requested until the source, revision, licence, exact size, digest, destination, and disk requirement have been read from the catalog and the user has confirmed the digest they were shown; the confirmation is compared against the catalog before any connection is opened. A download then checks free space, writes a `.partial` no concurrent attempt can collide with, checks for cancellation between chunks, and is promoted to its destination name only after its size and digest both match exactly. A cancellation, a short or overlong artifact, a wrong digest, a transport failure, or a full disk leaves the `.partial` where the user can see it and the destination untouched. An already-installed artifact that matches the catalog is reported ready without a request, and one that does not match is reported and left exactly as it is rather than replaced. Leftover `.partial` files are reported and never deleted automatically.
- The model transport is named by a `model-download` feature on `scrybe-meeting-application`, off by default there so the library and command-line graphs carry no network crate, mirroring how `scrybe-meeting-core` gates `openai-compat`. The desktop host's feature of the same name turns it on and is **on by default**, because the shipped application offers an in-app model download and a build without the transport cannot perform one. Every property of the manager other than the transport itself is still exercised in a build that carries no network crate at all. `scripts/check-app-egress-baseline.py` is the gate for the desktop graph the existing egress audit does not reach: it asserts the shipped host graph carries exactly the approved transport and nothing more — so a gained crate and a lost one both fail — that the host built without the feature carries none of it, which is what proves the feature genuinely controls the transport rather than labelling it, and that the catalog names one destination, over HTTPS, at the approved host, pinned to a revision its URL carries.
- A session library in the application. Sessions lists what is under the storage root grouped by the day each meeting was recorded, newest first, with unfinished and repair-needed sessions marked in words as well as by colour and a session that never wrote a start time filed under its own heading rather than dropped. Search matches folder names and titles for every session and notes and transcript text for completed ones. Both views re-read the root when the window is given attention back, through one watcher shared by every view rather than one per view, so a recording finished elsewhere or a folder moved in Finder shows as it is rather than as it was when the view opened. Neither view can edit, delete, archive, move, or tag anything.
- Searching again abandons the search still running. A cancellation token does not serialize and `invoke` has no abort, so a search is named instead: the view mints an identifier, passes it with the query, and hands the same string to a `cancel_query` command when it gives up. Cancellation stops work rather than deciding which answer is rendered — a cancel and the query it names are two calls and may run in either order — so the view also discards any answer whose identifier is no longer the one it is waiting for. The commands that walk the storage root run off the thread the window draws on, which they previously did not.
- Opening a session shows its state, identity, times, length, capture layout, providers, and which of the five artifacts are present and which are not. Notes are shown first; the transcript is read fifty lines at a time and the whole document never reaches the window. Five actions, each offered only when the session accepts it and each saying why beside itself when it does not: regenerate notes, repair the recording, reveal the folder, and copy either document. Reveal and both copies are commands — the folder is resolved in Rust beneath the storage root and the documents are read and put on the clipboard there, so the transcript is never assembled in the window for the same reason it is never read there whole. Every session a command names arrives as a parsed identity rather than a string the command validates.
- Playback audio is served to the window by a `scrybe-audio://` scheme that answers to one path shape and opens one file. The session identity in the path is parsed into the same root-confined type every command takes, and the only artifact reachable is `playback.opus`; a request for a transcript, for a traversal, or for an absolute path is refused before anything is opened. Full and byte-range responses are supported, with range parsing written out because the toolkit supplies none: an unparseable `Range` is ignored and the whole artifact served, an unsatisfiable one is refused with the artifact's size, and several ranges at once are answered with the whole representation. A session that never finished is refused as unfinished rather than as absent. The player is offered only for a session whose playback artifact exists.
- A session that completed a mono capture is no longer reported as having playback audio it does not have. Availability was derived from `audio.opus` existing on a completed session, but `playback.opus` is written only for a two-channel capture; of the four sessions in the configured storage root, three carry `audio.opus` with no `playback.opus` and all three were misreported. Availability now reads the artifact's own presence, and the cached scan's fingerprint watches that file too, so a merge that writes it is not invisible until something else in the folder moves.
- The shipped content security policy carries a `media-src` directive admitting the playback scheme. It previously carried no `media-src` at all, and `default-src` does not supply one for a custom scheme. The qualification script's expectation is a constant the same commit edits, so the string comparison alone would pass for any value written in both places: `scripts/qualify-desktop-app.py --hermetic --scenario library` is what closes that, driving the real webview at a real URL on the scheme and asserting what the protocol handler served. Admitting the scheme under `connect-src` instead passes every string comparison in that file and serves nothing; the scenario fails it.
- The configured notes generator moved from the command-line tool into `scrybe-meeting-application`, beside the trait it implements, so the tool and the application produce notes the same way rather than from two copies of segmentation, capping, and map-reduce policy. It sits behind a `notes-generation` feature, off by default in the library so its egress baseline still resolves a graph with no network crate, and on by default in the desktop host because the shipped application offers the action. `scripts/check-app-egress-baseline.py` now names every feature that buys a transport rather than one, so a graph that still linked a client could not pass a check whose label said otherwise.
- Settings is editable. It reports the same five readiness answers guided setup does, writes the fields the service layer owns, shows managed model storage with the same confirmed, cancellable, verified download — whose failures are distinguished, so a full disk reads differently from a corrupted download and is not offered a retry that could not work — and lists what diagnosis found. Saving rewrites only the named fields, and says so: comments, ordering, and any advanced block the form does not model survive an accepted change, and a change the schema rejects leaves the previous file exactly as it was. A successful save is confirmed on screen, and a field edited afterwards retires the confirmation so it never describes something other than what is shown. `Open advanced configuration` opens the configuration file for everything no form models, and reports it when the file cannot be opened — which on a fresh install is the ordinary case, because it does not exist yet; hosted-provider credentials live in environment variables named there and are never read into the window.
- Diagnostics repairs run only when chosen. Opening the page reads and applies nothing, which the page states and a test asserts by counting service calls. A finding whose recovery this application performs elsewhere — installing a model, which goes through the confirmation flow, or granting a permission, which happens in System Settings — is a pointer to where rather than a button that would reach a repair carrying no confirmation. Severity is carried by text as well as colour.
- `scripts/qualify-desktop-app.py` gains a `setup` scenario that drives the real application against an artifact source on loopback: the catalog is read without fetching, an unconfirmed install is refused with the fixture seeing no request, an artifact that does not fit is rejected before any request, a digest mismatch and a cancellation each leave the destination untouched and their partial where the user can see it, a confirmed install is promoted only after verification — sampled while it runs, so the destination name is never observed at a size other than the full one — and an artifact already installed is ready with no request at all. It builds the shape the application ships — default features, transport included — as every scenario now does; there is no per-scenario feature selection, because a scenario that qualified a feature selection nobody installs would establish nothing about the binary they do. Its no-network proof is therefore the socket sampler, asserting every destination the process reached was the fixture on loopback on the fixture's own port and that some socket was seen at all, together with the shipped content security policy. Only an explicit loopback address counts: a wildcard is a socket bound to every interface on the machine, and accepting one made that proof hollow. The `lifecycle` scenario asserts the complementary property on the same build — that the compiled-in client stays unused unless something asks it to act — and that its graph carries exactly the approved transport and nothing more.
- Guided first-run setup. Four steps — Welcome, Recording, Transcription and notes, Ready — reached from the sidebar, and the step the window opens on when this installation cannot record. Which is derived from readiness rather than from a completion flag, so an installation whose model was deleted is taken back to setup rather than shown a session list that cannot record. A revoked capture permission is not among the things that can take it back: nothing in this release detects one, and Ready reports capture as not checked rather than claiming it works. Welcome names the storage root, the ordinary files a session becomes, and the one network request setup can make, before the step that would make it. Recording explains the microphone and Screen & System Audio Recording permissions in terms of what each one captures — audio only, no video frames, no screenshots — and links each to its own System Settings pane for the case where one was already refused; it raises no dialog of its own, so it cannot raise one repeatedly. Transcription and notes shows the model's source, revision, licence, exact byte count, SHA-256, destination, and disk requirement, and requests nothing until the confirmation beneath them is pressed; progress is determinate and cancellable, a cancelled or failed download says plainly that nothing was installed and offers a retry, and an already-installed model is reported rather than offered again. Ready reports capture, transcription, notes, storage, and privacy separately, in words rather than by colour alone, and its verdict says what was checked rather than that the Mac is ready — capture permission is reported as not checked, because no probe in this application reads one without prompting. Every step carries a visible exit that leaves the application usable for reading and searching what is already recorded.
- Setup collects no hosted credential and contains no field that could take one. Notes being unavailable is stated as acceptable rather than as a failure: recording and transcription can be ready while notes are visibly not, notes need a language model running on this Mac, and setup can be finished without one and it configured later. What a recording made in that state produces is not claimed, here or in the wizard: the recording surface is not built yet.
- Local transcription and local notes readiness are reported separately, under the diagnostics component reserved for providers. A configuration naming local transcription with no model installed is an error naming the install action rather than a failure part way through the first recording, and a file at the model's destination that the catalog does not describe is reported and left exactly as it is. An interrupted model download is reported as its own finding, distinct from an interrupted session write under the storage root, and neither is deleted. A configured *loopback* notes endpoint is now dialled with a short timeout rather than only parsed, so a configuration pointing at a local provider that is not running says so; a remote endpoint is never dialled, because reaching out to see whether a hosted provider answers would be an egress with no mandate behind it.
- A typed readiness snapshot reports capture, transcription, notes, storage, and egress separately, derived from one read-only diagnosis rather than from a second set of probes. Notes readiness is deliberately not a gate: a recording may begin with notes unavailable, and egress is disclosure rather than a fault. Transcription that runs from a hosted provider reports as not-locally-configured rather than as blocked, because its readiness depends on a credential this application never handles and an inability to assert it is not a fault.
- The settings surface may now write `[capture].mic_device`, `[capture].hotkey`, and the list-valued `[shell].indicators`, which had no representable value before. The closed set of writable fields still contains no credential-bearing key, and an update still edits the document in place — comments, ordering, and blocks no form models survive an accepted change, and the previous file survives a rejected one.
- The desktop host reaches these through ten further commands, each granted by its own capability file entry and each forwarding to a service method. Reading and mutating are separate commands throughout: a diagnostics screen calls the read, and only an explicit action calls the repair. Acquiring a model is two commands in order — the offer a confirmation prompt is built from, then the install that refuses a digest the prompt did not show. The two commands that hand the user to a platform surface take a capability name or nothing at all; their destinations are derived in Rust, so the interface cannot ask the host to open something it chose.
...
- One version, written once. `[workspace.package]` now declares it and every workspace member takes it with `version.workspace = true`, where nine manifests previously each hardcoded the same string and agreed only by discipline. Three classes of surface cannot inherit and were each restating it: `scrybe-desktop/src-tauri`, which declares its own workspace so the root's jobs can build the library and command-line graph without a WebView host in them; `tauri.conf.json`, a JSON literal, which is the value stamped into the bundle's `CFBundleShortVersionString`; and eleven intra-workspace `=` pins, which a published manifest needs alongside its path and which have to move in lockstep or `cargo package` stops resolving. `scripts/check-version-agreement.py --write` propagates into all three, and the same script's default check holds thirty-two source surfaces to it and names the file behind any disagreement. Given `--bundle` it adds two more — the two version keys a built application carries in its own `Info.plist` — and those are the only two of the thirty-four that measure the artifact rather than the source. The desktop application previously reported `CFBundleShortVersionString = 0.0.0`, a placeholder unrelated to anything the command-line tool shipped; it now reports the workspace version. No version number changed.

- The publish set and its order are derived from the workspace rather than restated. `.github/workflows/ci.yml`'s `dist-plan` job held a typed list of six packages, `docs/RELEASE.md` held a second typed list of five, and the two disagreed: the `RELEASE.md` list omitted `scrybe-widgets`, which `scrybe` names with an exact version and which therefore has to be on the registry before `cargo publish -p scrybe --locked` can resolve. The same omission in `ci.yml` is what broke `cargo package -p scrybe`. `scripts/publish-order.py` now reads the set from each manifest's own `publish` field and orders it by the dependency edges between the packages in it, so marking a package publishable changes the set with nobody editing a list. It refuses three things outright: a publishable package that depends on a private workspace member — the `scrybe-widgets` failure stated as a check, which the old list could not notice because it *was* the thing that did not notice it — a dependency cycle, and a derived set that `docs/crates-io-registry-state.toml` does not cover exactly, so a new publishable package forces its registry state to be looked up in the same change rather than at publication time. `docs/publish-order.md` is the generated artifact a human reads before authorizing anything, and it leads with the fact that matters most to that decision: `scrybe-meeting-application` and `scrybe-widgets` have never been published, so whenever a release version is assigned, their first publication happens as part of it — permanent, unrepeatable, with no prior version to diff against. `dist-plan` packages the derived set and asserts the artifact is not stale.

- The signature, Gatekeeper, and notarization assertions exist, and none of them can pass without a real credential. `scripts/check-signed-artifact.py` reads the `Authority` and `TeamIdentifier` lines out of `codesign -dvvv` and requires a Developer ID Application identity whose two statements of its own team agree, parses the verdict word out of `spctl -a -vv --type execute`, and reads the status Apple returns for a submission together with a ticket Apple issued. It calls `codesign --verify` nowhere: an ad-hoc-signed bundle built from this repository returns exit 0 from `codesign --verify --deep --strict` while `spctl` reports `rejected`, so a gate built on it certifies an artifact Gatekeeper refuses to run. `spctl`'s exit status is not read either — the same `rejected` verdict measured at exit 3 here and exit 0 elsewhere, so an assertion written against it would have passed in one environment and failed in the other while the artifact was equally unshippable in both. A missing credential exits 3 rather than 1, because “nobody has configured signing” and “this signed artifact is bad” are different answers and collapsing them is how an unsigned artifact acquires a claim it was checked.
- The unsigned bundle path is no longer silent. `packaging/macos-app/build-app.sh` made signing optional: with no flags it printed a warning, built the bundle anyway, and finished with a `codesign --verify` that could not have caught the result — so the ordinary way to run it produced an unsigned bundle nothing downstream refused. Exactly one signing mode is now required with no default: `--sign` for a Developer ID, which is the only shippable one and which is asserted afterwards by reading the signing authority by name; `--sign-self` and `--unsigned` for local development, both of which state on stdout that their output is not shippable. `--sign` refuses before signing when no matching certificate is in any keychain, naming the absent certificate and listing what the machine does have, and refuses an identity that is not a Developer ID Application identity at all — `--sign -` is ad-hoc signing, which names nobody and is exactly the artifact `codesign --verify` calls valid. A bundle that was signed and still fails the assertions is refused, so signing having happened is no longer evidence that anything shippable came out.

- `scripts/qualify-desktop-app.py` gains an `installed` scenario that drives a copy of the bundle from outside the build tree, and it is the one scenario that does not pass. It checks something the other four assume: that the process being driven came out of the copy that was installed, read from `NSRunningApplication` rather than inferred from the path that was launched — `open` goes through Launch Services, which is free to hand a launch to any registered copy of the identifier. It records a session and then asks the reader for that session, which is the handoff neither `recording` nor `library` covered: one asserts what the pipeline wrote to disk and the other reads sessions the harness seeded, so nothing had ever asserted that the reader can open what the recorder produced. And it reads the tray's accessible names out of the running application's accessibility tree, which is the one accessible surface of this application that is not markup; renaming a tray action in the host fails it, so it reads the application rather than a constant this file also declares. It then stops at two walls, both as failures rather than skips: a TCC Accessibility grant, which is a human grant this harness must not give itself because granting it to a terminal grants it to everything run from that terminal, and a Developer ID Application certificate with a notarization credential, which nobody here has. A qualification that reported an installed application as qualified, having driven an artifact nobody else could install, would be worse than no qualification, so a green `installed` on a machine with no distribution credential means a check has stopped being able to fail. The existing four scenarios are unchanged and still report 20, 37, 39, and 46 checks.
- A failing qualification run reports how many checks it ran, not only how many failed. One refusal out of forty and one out of three read identically before, and the `installed` scenario is designed to end on a failure, so its coverage would otherwise never be printed at all.

### Fixed

- The desktop application resolves a `~`-prefixed storage root against the home directory. It previously passed the configuration's value through unexpanded while the configuration written on a real installation holds the literal `~/scrybe`, so the shipped application resolved a relative directory named `~` beneath wherever it was launched from and presented an empty session list over a storage root full of recordings. The command-line tool has had this expansion all along; the two now agree. A `~user` form is still left alone, because resolving it to the current user's home would point the application at the wrong person's files.
...
- The desktop dependency graphs are covered less completely than the root graph, and this records exactly where. `cargo audit`, `cargo deny`, and `cargo vet` do read `scrybe-desktop/src-tauri/Cargo.lock`, but three gaps remain. `scripts/check-egress-baseline.py` resolves only the root graph, so the desktop graph is not checked for networking crates by anything; the three crates from its denylist that the desktop lockfile carries — `reqwest`, `hyper`, and `hyper-util` — are reached only through `tauri`'s `cfg(any(target_os = "android", all(target_vendor = "apple", not(target_os = "macos"))))` dependency table, so a macOS build never compiles them and `cargo tree --target aarch64-apple-darwin` resolves none of them, but a networking crate that was not target-gated would be equally invisible. The 624 packages in `scrybe-desktop/pnpm-lock.yaml` have no advisory scan and no licence gate on any branch. `cargo vet` is advisory rather than blocking on both graphs, with 472 unvetted crates in the desktop graph and 397 in the root graph.
- The `cargo audit` ignore for RUSTSEC-2024-0421 now carries desktop-graph evidence, where it previously carried a root-graph argument that did not transfer. The advisory is vulnerability-class against `idna 0.5.0` and fires on the desktop lockfile, which reaches it at run time through `tauri 2.11.5 → url 2.5.0`, not through the `Hook::Git` chain the existing justification reasons about. Its escalation shape — host-name comparison used as a privilege check — is exactly the shape of the desktop navigation guard. What bounds it is that a release build admits only the `tauri` scheme, whose host is served by Tauri's embedded asset protocol and never resolved through DNS; the `http://localhost` branch a masked Punycode label could otherwise reach is compiled only into debug builds, which are not distributed. The remedy is unchanged and shared: the `cargo update` to `url 2.5.4` / `idna 1.x` that the root entry already waits on regenerates both lockfiles.
...
Closes the v1.0.3 → v1.1 follow-up at the top of `.docs/handoff.md` §3.1: `notes.md` was still generated by the `StubLocalLlm` template body, so the summary pipeline summarized nothing. v1.0.4 wires the existing `OpenAiCompatLlmProvider` (live since pre-v1.0, but never reachable from `scrybe record`) through a config-driven dispatch so a `cargo install --features llm-openai-compat` build with `--llm openai-compat` produces real `notes.md` summaries via any OpenAI-compatible `/chat/completions` endpoint — Ollama at `http://localhost:11434/v1` by default, plus vLLM, OpenAI, Groq, Together.

The publish posture from v1.0 carries forward unchanged: only `scrybe` (the placeholder) publishes to crates.io. No Tier-1 surface changes — the `LlmProvider` trait shape is untouched; the addition is `OpenAiCompatLlmProvider::from_config(&LlmConfig)` (Tier 2: `scrybe-core` public surface) plus a CLI-side `CliLlm` enum-dispatch wrapper that mirrors v1.0.1's `CliStt` pattern. The choice to reuse the existing OpenAI-compat path against Ollama's `/v1/chat/completions` endpoint, rather than wire a separate native `OllamaLlmProvider` against `/api/generate`, is documented in `.docs/handoff.md` §3.1: Ollama exposes the OpenAI-compat shape natively, the existing provider already has 6 wiremock-backed tests for retry / transient-status / empty-choices / no-auth paths, and reusing it avoids duplicating ~350 LoC of HTTP transport-and-retry code for a payload that's strictly less expressive.

### Added

- `scrybe-core::providers::OpenAiCompatLlmProvider::from_config(&LlmConfig)` constructor that maps the user-facing `[llm]` config block (already present since v0.5 per the `LlmConfig` schema in `scrybe-core::config`) onto `OpenAiCompatLlmConfig`. Reads `api_key_env` from the process environment at construction time (empty when unset, which `try_complete` interprets as "omit `Authorization` header" — the documented Ollama / self-hosted vLLM path). The `display_name` reported in `meta.toml [providers].llm` is `"<provider>:<model>"` (e.g. `ollama:llama3.1:8b`, `openai-compat:llama3-70b-8192`) so users can distinguish a Groq run from an Ollama run at a glance.
- `scrybe-cli` `llm-openai-compat` cargo feature forwarding to `scrybe-core/openai-compat`. Off by default so the air-gappable default-feature graph stays network-free per the egress audit (`scripts/check-egress-baseline.py`); enable on opt-in via `cargo install --features llm-openai-compat,...`. Without the feature, `--llm openai-compat` errors at start time naming both the flag and the missing feature so the user knows what to rebuild with — same hard-error pattern as `--whisper-model` without `--features whisper-local` (v1.0.1).
- `scrybe record --llm <stub|openai-compat>` flag value. New `LlmBackendArg` enum surfaced to clap with `stub` (default; matches v1.0.3 behavior) and `openai-compat` (literal token via `#[value(name = "openai-compat")]`). When `openai-compat` is selected, `build_llm_provider` reads `cfg.llm` (loaded via `load_or_default_config()` at the start of `run_with_stop`) and constructs `OpenAiCompatLlmProvider::from_config`. The retry policy in `[llm].retry` covers transient 429 / 5xx upstream failures with exponential backoff; permanent 4xx short-circuits without retries.
- `scrybe-cli` `CliLlm` enum-dispatch wrapper (`Stub(StubLocalLlm)` / `OpenAiCompat(OpenAiCompatLlmProvider)`) mirroring the v1.0.1 `CliStt` pattern. Variants stay `Sized` so the existing `SessionInputs<L: LlmProvider>` generic does not need a `?Sized` relaxation in `scrybe-core` for this v1.0.x patch — same precedent rationale as `CliStt`.
- Three new unit tests on `scrybe-core::providers::openai_compat_llm`: `test_from_config_composes_display_name_as_provider_colon_model`, `test_from_config_treats_missing_api_key_env_as_empty_string`, `test_from_config_reads_api_key_from_named_env_var`. The env-var test uses a long unique var name and unsets it after the assertion so concurrent test runs do not collide.
- Four new unit tests on `scrybe-cli::commands::record`: `test_build_llm_provider_returns_stub_when_backend_is_stub`, `test_build_llm_provider_errors_when_openai_compat_requested_without_feature` (cfg-gated to no-feature builds; mirrors the v1.0.1 STT-feature-gate test), `test_build_llm_provider_constructs_openai_compat_when_feature_enabled` (cfg-gated to feature-on; asserts the `<provider>:<model>` name flows through), `test_llm_backend_arg_default_is_stub`.
- `INSTALL.md` "Real notes summaries via Ollama / OpenAI-compat" section: full build invocation, sample `[llm]` config block (Ollama default + Groq override), runtime `--llm openai-compat` example, env-var-driven API key loading, fallback behavior when the feature is off.

### Changed

- All workspace crates bump from `1.0.3` to `1.0.4`. Path-dep version pins follow.
- `scrybe-cli/src/commands/record.rs` swaps the hardcoded `let llm = StubLocalLlm::new()` at line 195 for `let llm = build_llm_provider(args.llm, &cfg.llm)?` so the LLM provider is now selected at runtime by the new `--llm` flag rather than always-stub.
- `scrybe-core::providers::openai_compat_llm::OpenAiCompatLlmConfig::default().timeout` and `OpenAiCompatSttConfig::default().timeout` shift from `Duration::from_secs(120)` / `from_secs(60)` to `Duration::from_mins(2)` / `from_mins(1)` for the clippy `duration_suboptimal_units` lint that surfaced once the `llm-openai-compat` lane added these files to the maximal clippy gate. No behavior change — both forms construct identical `Duration` values.
- `scrybe-cli` LoC ceiling stays at 2500; current code-only LoC is 2486 with the new `CliLlm` enum + `build_llm_provider` + tests. `scrybe-core` LoC ceiling stays at 9000; current code-only LoC is 8861 with `from_config` + tests.
...
- `cargo audit` and `cargo deny` policies are unchanged from v1.0.3. The `llm-openai-compat` feature pulls in the same `reqwest` dep that `openai-compat` already declared on `scrybe-core` since v0.5; no new transitive crates entered the graph.
- Egress audit clean: `llm-openai-compat` is gated, so the default-feature graph is unchanged from v1.0.3 (`scripts/check-egress-baseline.py` reports 92 packages, no networking crates).
- API keys load from named env vars rather than from on-disk config, so a config-leak does not leak credentials. The CLI never logs the resolved key value.

### Known limitations

- **`audio.opus` interleaves mic + system frames by arrival time.** Stereo encoding is a v1.0.x → v1.1 deliverable (unchanged from v1.0.3).
- **System audio capture is macOS-only.** Linux PipeWire and Windows WASAPI loopback adapters are v1.0.x → v1.1 deliverables (unchanged from v1.0.3).
...
- **Native Ollama `/api/generate` provider not added.** v1.0.4 reaches Ollama via its OpenAI-compat endpoint (`/v1/chat/completions`) rather than the native `/api/generate` shape, since both routes need `reqwest` and the OpenAI-compat path is already tested. A native `OllamaLlmProvider` remains a future option if the simpler payload becomes load-bearing for a specific consumer.

### Workspace

- 8 crates (unchanged).
- Publish posture unchanged.
- Test counts: 508 default / 526 with `--features llm-openai-compat` (the openai-compat feature pulls in 18 additional tests in `scrybe-core` covering the live `OpenAiCompatSttProvider` + `OpenAiCompatLlmProvider`).

### Contributors

- Maintainer: Mathews Tom.

[1.0.4]: https://github.com/Mathews-Tom/scrybe/releases/tag/v1.0.4
