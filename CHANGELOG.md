...
- Settings is editable. It reports the same five readiness answers guided setup does, writes the fields the service layer owns, shows managed model storage with the same confirmed, cancellable, verified download, and lists what diagnosis found. Saving rewrites only the named fields, and says so: comments, ordering, and any advanced block the form does not model survive an accepted change, and a change the schema rejects leaves the previous file exactly as it was. `Open advanced configuration` opens the configuration file for everything no form models; hosted-provider credentials live in environment variables named there and are never read into the window.
