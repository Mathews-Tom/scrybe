...
- The model transport is gated behind a default-off `model-download` feature on `scrybe-meeting-application`, enabled only by the desktop host's own default-off feature of the same name, mirroring how `scrybe-meeting-core` gates `openai-compat`. Every property of the manager other than the transport itself is exercised in a build that carries no network crate at all. `scripts/check-app-egress-baseline.py` is the gate for the desktop graph the existing egress audit does not reach: it asserts the default graph carries none of the denylisted HTTP, TLS, DNS, QUIC, or WebSocket crates, that the graph with the feature enabled carries exactly the approved transport and nothing more, and that the catalog names one destination, over HTTPS, at the approved host, pinned to a revision its URL carries.
