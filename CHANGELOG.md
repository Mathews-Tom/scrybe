...
- The interface reaches Rust through four named read commands and one recording-status event, and through nothing else. It cannot run a shell command, read a path, or call anything its capability files do not grant; those files are scoped to the one window by name and grant no filesystem, shell, network, process, or path permission. `pnpm --dir scrybe-desktop run check:capabilities` rejects a capability that reaches any of them, and `pnpm --dir scrybe-desktop run check:bindings` fails when the TypeScript the interface imports no longer matches the Rust it was generated from.
