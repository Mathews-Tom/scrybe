...
- `scrybe_core::pipeline::interleave::StereoInterleaver` — buffers per-source mono samples in bounded ring buffers, emits aligned interleaved L/R PCM as soon as both sides have data, and zero-fills the lagging side once one source lags beyond a configurable tolerance (default 200 ms). Out-of-range frames (wrong sample rate, non-mono, `FrameSource::Mixed`) fail fast with `PipelineError::InvalidFrame` rather than silently producing corrupted output.
