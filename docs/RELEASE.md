...
Confirm main CI is green and the release commit is the expected v2.2.0 preparation commit:
...
```sh
cargo info scrybe-meeting-core@2.2.0 --registry crates-io
```
...
```sh
cargo info scrybe-meeting-application@2.2.0 --registry crates-io
```
...
```sh
cargo info scrybe-meeting-capture-mac@2.2.0 --registry crates-io
cargo info scrybe-meeting-capture-mic@2.2.0 --registry crates-io
```
...
```sh
cargo info scrybe-widgets@2.2.0 --registry crates-io
```
...
Both version commands must report `scrybe 2.2.0`. `doctor` and `record --help` must execute without a repository checkout. Run Doctor from a terminal and decline the optional live permission probe during this registry-only acceptance; the release's hardware qualification covers the real probes separately.
...
Expected v2.2.0 asset names include:
...
- `SHA256SUMS.txt`
- `Scrybe_2.2.0_aarch64.dmg`
- `Scrybe_2.2.0_x86_64.dmg`
- `Scrybe_2.2.0_aarch64.app.tar.gz`
- `Scrybe_2.2.0_aarch64.app.tar.gz.sig`
- `Scrybe_2.2.0_x86_64.app.tar.gz`
- `Scrybe_2.2.0_x86_64.app.tar.gz.sig`
- `latest.json`
...
If the GitHub workflow fails before publishing a usable release, fix the workflow on main and cut a new patch version. Do not retag v2.2.0 after crates.io publication because the registry package and source tag must remain permanently aligned.
