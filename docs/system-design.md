...
| CLI subcommands | `scrybe init`, `record`, `rec`, `list`, `show`, `doctor`, `bench`. Flag and exit-code semantics are stable; output formatting is Tier 3. `rec` added at v1.1.0-rc1 — the explicit-flags entry point `record` carried until then, freeing `record` to become the ergonomic `scrybe record TITLE` form per `.docs/DEVELOPMENT_PLAN.md` M1 |
