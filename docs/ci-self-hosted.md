...
> **Empirical note.** PR #20's dispatch run on macOS 26.4.1 with the `DRUK-scrybe` MacBook Pro runner passed the live Core Audio Taps E-1 test (`peak > 0.01` noise floor) without the GUI grants in this section having been performed during the session that registered the runner. `Runner.Listener` obtained Audio Capture authorization on that host; the mechanism is not characterized here (responsible-process inheritance, an absence of TCC gating for this specific call on this macOS version, or a pre-existing grant attributed via some other path are all candidates). Treat this section as the documented fallback: if the live test passes on your host without the GUI step, skip ahead to step 4; if a future macOS update tightens TCC and the test fails with `kAudioHardwareUnauthorized`, come back here.
