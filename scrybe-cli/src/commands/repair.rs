...
mod tests {
    ...
    async fn test_run_refuses_a_session_whose_lock_carries_no_readable_pid() {
        ...
        // A journal makes the folder a session the repository resolves,
        // so the command reaches the lock check rather than failing to
        // find anything to repair.
        std::fs::create_dir_all(folder.join("journal")).unwrap();
        std::fs::write(folder.join("journal").join("manifest.toml"), b"").unwrap();
