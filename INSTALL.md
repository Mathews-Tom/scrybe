...
```sh
TAG=v2.3.1   # the release you are verifying
...
```sh
git clone --branch v2.3.1 https://github.com/Mathews-Tom/scrybe.git scrybe
cd scrybe
...
Linux and Windows recording remain parked until each platform has a maintainer-owned hardware qualification path. The v2.3.1 crates.io installation contract is macOS-only; do not present a successful cross-platform compile as recording support.
