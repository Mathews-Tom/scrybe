...
- The notes step uses the stub LLM by default. Pass `--llm openai-compat` (and build with `--features llm-openai-compat`) to generate real `notes.md` summaries via any OpenAI-compatible `/chat/completions` endpoint — see "Real notes summaries via Ollama / OpenAI-compat" below.
...
`notes.md` is generated by the LLM provider at `SessionEnd` (one prompt → one completion, per `docs/system-design.md` §4.3). The default backend is `stub`, which writes a fixed templated body so CI smoke tests stay hermetic. To produce real summaries, build with `--features llm-openai-compat` and pass `--llm openai-compat`:
...
# Configure [llm] in your config (defaults to Ollama on localhost; the
# block lives in ~/Library/Application Support/scrybe/config.toml on
# macOS, $XDG_CONFIG_HOME/scrybe/config.toml elsewhere).
cat <<'EOF' >> ~/Library/Application\ Support/scrybe/config.toml
[llm]
provider = "ollama"
...
# With Ollama running on localhost:11434
ollama pull llama3.1:8b
ollama serve &

# Record a session with real summarization
scrybe record \
  --title "client-call" \
  --source mic+system \
  --whisper-model ~/Library/Application\ Support/scrybe/models/ggml-base.en.bin \
...
`scrybe record --llm openai-compat` then reads `$GROQ_API_KEY` from the process environment at start time. An empty / unset env var sends no `Authorization` header (the documented Ollama / self-hosted vLLM path).
